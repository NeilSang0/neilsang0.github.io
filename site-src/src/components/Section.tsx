...
export default function Section({ id, tone, title, kicker, z, children, className = '' }: { id: string; tone: 'dark' | 'light'; title: string; kicker?: string; z: number; children: ReactNode; className?: string }) {
  ...
  return (
    <section id={id} className={`rounded-t-[40px] sm:rounded-t-[50px] md:rounded-t-[60px] -mt-10 sm:-mt-12 md:-mt-14 px-5 sm:px-8 md:px-10 pt-20 sm:pt-24 md:pt-32 pb-24 sm:pb-28 md:pb-36 ${className}`} style={style}>
      <FadeIn as="h2" y={40} className={`${dark ? 'hero-heading' : 'text-[#0C0C0C]'} font-black uppercase text-center leading-none tracking-tight`} style={{ fontSize: 'clamp(3rem, 12vw, 160px)' }}>
        {title}
      </FadeIn>
      {kicker && (
        <FadeIn as="p" delay={0.1} className={`${dark ? 'text-[#D7E2EA]' : 'text-[#0C0C0C]'} text-center font-light uppercase tracking-widest mt-4 sm:mt-6`} style={{ fontSize: 'clamp(0.7rem, 1.2vw, 0.95rem)', opacity: 0.6 }}>
          {kicker}
        </FadeIn>
      )}
    ...
    <ol className="max-w-5xl mx-auto">
      {items.map((it, i) => (
        <FadeIn as="li" key={it.title + i} delay={i * 0.1} className="flex gap-6 sm:gap-10 md:gap-14 items-start py-8 sm:py-10 md:py-12" style={{ borderTop: rule(tone), borderBottom: i === items.length - 1 ? rule(tone) : undefined }}>
          <span className={`${tone === 'dark' ? 'hero-heading' : 'text-[#0C0C0C]'} font-black leading-none flex-none`} style={{ fontSize: 'clamp(3rem, 10vw, 140px)' }}>{String(i + 1).padStart(2, '0')}</span>
          <div className="flex flex-col gap-3 pt-2 min-w-0">
            <h3 className="font-medium uppercase leading-tight" style={{ color: c, fontSize: 'clamp(1rem, 2.2vw, 2.1rem)' }}>
            ...
            {it.sub && <p className="font-light uppercase tracking-widest" style={{ color: c, opacity: 0.6, fontSize: 'clamp(0.7rem, 1.1vw, 0.9rem)' }}>{it.sub}</p>}
            {it.text && <p className="font-light leading-relaxed max-w-2xl" style={{ color: c, opacity: 0.6, fontSize: 'clamp(0.85rem, 1.6vw, 1.25rem)' }}>{it.text}</p>}
            {it.tags && (
