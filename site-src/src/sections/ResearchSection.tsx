          ...
          <FadeIn key={p.id} delay={i * 0.15} className="rounded-[40px] sm:rounded-[50px] md:rounded-[60px] border-2 border-[#D7E2EA] p-6 sm:p-8 md:p-12 flex flex-col gap-6 md:gap-8">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="min-w-0 max-w-3xl">
                ...
                <h3 className="hero-heading font-black leading-tight mt-2" style={{ fontSize: 'clamp(1.2rem, 2.6vw, 2.3rem)' }}>{p.title}</h3>
              </div>
              <LiveProjectButton href={p.href} label="Read on SSRN" />
            </div>
            <p className="text-[#D7E2EA] font-light leading-relaxed max-w-3xl" style={{ fontSize: 'clamp(0.85rem, 1.3vw, 1rem)', opacity: 0.75 }}>{p.abstract}</p>
            <div className="flex flex-wrap gap-8 sm:gap-12 pt-4" style={{ borderTop: '1px solid rgba(215,226,234,0.15)' }}>
              {p.stats.map((s) => (
                <div key={s.label}>
                  <div className="hero-heading font-black leading-none" style={{ fontSize: 'clamp(1.7rem, 3.8vw, 3.3rem)' }}>{s.value}</div>
